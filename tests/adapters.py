...
from transformer.modules import Linear, Embedding, RMSNorm, Swiglu, RoPE, softmax, Attention, MultiHeadedCausalSelfAttention, TransformerBlock, TransformerLM, cross_entropy, AdamW, cosine_annealing_lr, gradient_clipping, get_batch, save_checkpoint, load_checkpoint
...
def run_swiglu(
    ...
    swiglu = Swiglu(d_model, d_hidden=d_ff)
...
def run_scaled_dot_product_attention(
    ...
    return attn_mod.forward(Q, K, V, mask)
...
def run_multihead_self_attention(
    ...
    multi_headed_self_attn = MultiHeadedCausalSelfAttention(d_model, num_heads, max_seq_len=in_features.shape[-2])

    multi_headed_self_attn.load_state_dict({
        "Q.W": q_proj_weight,
        "V.W": v_proj_weight,
        "K.W": k_proj_weight,
        "O.W": o_proj_weight
    ...
    return multi_headed_self_attn.forward(in_features)
...
def run_multihead_self_attention_with_rope(
    ...
    Returns:
        ...
        implementation with the given QKV projection weights and input features.
    """
    multi_headed_self_attn = MultiHeadedCausalSelfAttention(d_model, num_heads, max_seq_len=max_seq_len, theta=theta)
    ...
    return multi_headed_self_attn.forward(in_features, token_positions)
...
def run_rope(
    ...
    """
    rope = RoPE(theta, max_seq_len, d_k)
...
def run_get_batch(
    ...
    return get_batch(dataset, context_length, batch_size, device)
